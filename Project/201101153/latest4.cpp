...
//float cam_x=1.5f,cam_y=1.2f,cam_z=7.0f;//orig
float cam_x=1.5f,cam_y=1.2f,cam_z=11.0f;
//float cam_x=1.5f,cam_y=-1.0f,cam_z=50.0f;
...
if(level==1)
{
	...
	if((lookx+1.7>2) ||(lookx+1.7<-2.3) || lookz+4.2>4.4 || lookz+4.2<-2
			...
			//|| (lookz+4.2<-1.7+(10*0.45 && lookz+4.2>-1.7+(9.5-1)*0.45 
			//&& lookx+1.7 > -2+(7-1)*0.45 && lookx+1.7 <-2+(8 -1)*0.45)
			//|| (lookz+4.2<-1.7+(12-1)*0.45 && lookz+4.2>-1.7+(11.5-1)*0.45 
			//&& lookx+1.7 > -2+(6.5-1)*0.45 && lookx+1.7 <-2+(7 -1)*0.45)
			//|| (lookz+4.2<-1.7+(3-1)*0.45 && lookz+4.2>-1.7+(2-1)*0.45 
			//&& lookx+1.7 > -2+(3-1)*0.45 && lookx+1.7 <-2+(4 -1)*0.45)
	...
	*/
	...
	moretiles(2.5,-0.5,8);
	moretiles(3,-0.5, 8);
	moretiles(1+chalo,-0.5,8.5);//moving tile
	moretiles(1,-0.5,9);
	...
	if(lookz+9.2 > 8.3 && lookz + 9.2 < 8.7 && lookx+1 < 1.1 && lookx+1 > 0.9 && chalo<0.2 && chalo >-0.2 )
		//glTranslatef(move_x-chalo,move_y,move_z);
{
	//	glTranslatef(0,0,0);
imontile=1;
}
if(fall!=1 && imontile==0)
		glTranslatef(move_x,move_y,move_z);
...
*/
	...
	void initRendering() {
		...
		Image* image1 = loadBMP("neel.bmp");//base
		_textureId2 = loadTexture(image1);
		Image* image2 = loadBMP("mount.bmp");//mount //front
